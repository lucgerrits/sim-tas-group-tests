...
async function main() {
    ...
    var acc_arr = substrate_sim.accounts.getAll();
    if (process.argv[3] == "true") {
        for (let i = 0; i < acc_arr.length; i++) {
            // console.log(`Add car:\t ${acc_arr[i].address}`)
            await substrate_sim.send.new_car(api, substrate_sim.accounts.alice(), acc_arr[i]); //add acc_arr[i] as a car
        }
    }
    console.log("Done")
}
